...
function getRandomFrontendMirror() {
    return FRONTEND_MIRRORS[Math.floor(Math.random() * FRONTEND_MIRRORS.length)];
}
...
async function handleCategories() {
    ...
    const categories = (content.operatingList || []).map(item => ({
        title: item.title,
        path: item.path,
        type: item.type,
        opId: item.opId,
        itemCount: item.subjects?.length || 0
    })).filter(cat => cat.path);
...
async function handleCategory(categoryPath) {
    const response = await makeApiRequest(`${HOST_URL}/wefeed-h5-bff/web/home`);
    const data = await response.json();
    const content = processApiResponse(data);
    
    const category = (content.operatingList || []).find(item => item.path === categoryPath);
    
    if (!category) {
        return new Response(JSON.stringify({
            status: 'error',
            message: 'Category not found',
            hint: 'Use /api/categories to see all available categories'
        }), {
            status: 404,
            headers: {
                'Content-Type': 'application/json',
...
async function fetchSourcesData(movieId, season, episode) {
    ...
    if (!detailPath) {
        return { error: 'Could not get movie detail path' };
    }

    const frontendMirror = getRandomFrontendMirror();
    
    const refererUrl = `https://${frontendMirror}/spa/videoPlayPage/movies/${detailPath}?id=${movieId}&type=/movie/detail`;
    ...
    const response = await makeApiRequest(`${HOST_URL}/wefeed-h5-bff/web/subject/download?${params}`, {
        headers: {
            'Referer': refererUrl,
            'Origin': `https://${frontendMirror}`
        }
    });
    
    const data = await response.json();
    return { 
        content: processApiResponse(data),
        movieInfo
    };
}
...
async function handleSources(movieId, url, request) {
    ...
    const { content, movieInfo, error } = await fetchSourcesData(movieId, season, episode);
    ...
    if (content && content.downloads) {
        ...
        const sources = content.downloads.map(file => {
            ...
            if (isEpisode) {
                ...
                streamParams.append('season', season);
                streamParams.append('episode', episode);
            }
...
async function handleStream(url, request) {
    ...
    const movieId = urlObj.searchParams.get('movieId');
    const season = parseInt(urlObj.searchParams.get('season')) || 0;
    const episode = parseInt(urlObj.searchParams.get('episode')) || 0;
    ...
    let headResponse = await fetch(streamUrl, {
        method: 'HEAD',
        headers: {
            'User-Agent': 'okhttp/4.12.0',
            'Referer': `https://${frontendMirror}/`,
            'Origin': `https://${frontendMirror}`
        }
    });

    // If the URL is expired (403, 404), fetch fresh URL
    if ((!headResponse.ok || headResponse.status === 403 || headResponse.status === 404) && movieId) {
        console.log(`Stream URL expired for ${movieId}. Fetching fresh URL...`);
        const { content } = await fetchSourcesData(movieId, season, episode);
        
        if (content && content.downloads) {
            // Find the matching quality (or fallback to the first available)
            const matchingFile = content.downloads.find(f => (f.resolution || 'Unknown') === quality) || content.downloads[0];
            
            if (matchingFile && matchingFile.url) {
                console.log(`Successfully fetched fresh stream URL for ${quality}`);
                streamUrl = matchingFile.url;
                
                // Re-attempt HEAD request
                headResponse = await fetch(streamUrl, {
                    method: 'HEAD',
                    headers: {
                        'User-Agent': 'okhttp/4.12.0',
                        'Referer': `https://${frontendMirror}/`,
...
async function handleDownload(url, request) {
    const urlObj = new URL(url);
    let downloadUrl = urlObj.searchParams.get('url');
    const movieId = urlObj.searchParams.get('movieId');
    ...
    // If the URL is expired (403, 404), fetch fresh URL
    if ((!headResponse.ok || headResponse.status === 403 || headResponse.status === 404) && movieId) {
        console.log(`URL expired for ${movieId}. Fetching fresh URL...`);
        const { content } = await fetchSourcesData(movieId, season, episode);
        ...
        if (content && content.downloads) {
            ...
            if (matchingFile && matchingFile.url) {
                console.log(`Successfully fetched fresh URL for ${quality}`);
                downloadUrl = matchingFile.url;
                
                // Re-attempt HEAD request with the new fresh URL
                headResponse = await fetch(downloadUrl, {
                    method: 'HEAD',
                    headers: {
                        'User-Agent': 'okhttp/4.12.0',
                        'Referer': `https://${frontendMirror}/`,
    ...
    let fileSize = parseInt(headResponse.headers.get('content-length'));
    const contentType = headResponse.headers.get('content-type') || 'video/mp4';
        ...
        <div class="content">
            <div class="feature-box">
                ...
                <ul>
                    ...
                    <li>Browse by categories: Anime, K-Drama, Action, Horror, Kids & more</li>
            ...
            <div class="endpoint">
                ...
                <p>Explore content by categories like Anime, K-Drama, Action, Horror, Kids content, and more!</p>
                <span class="status">WORKING</span>
                <br><br>
                <a href="/api/categories" class="example-link">View All Categories</a>
                <a href="/api/category/0UA0warzA4" class="example-link">Anime</a>
                <a href="/api/category/IWgO1od5Tu8" class="example-link">Animated Film (Kids)</a>
                <a href="/api/category/QUfN2mjbYc7" class="example-link">Action Movies</a>
                <a href="/api/category/ejGpCRUQgE" class="example-link">K-Drama</a>
            </div>
            ...
            <div style="text-align: center; margin-top: 30px; padding: 20px; background: #f7fafc; border-radius: 10px;">
                ...
                <p><strong>All 9 endpoints operational</strong> with real MovieBox data</p>
                <p style="color: #666; font-size: 0.9em; margin-top: 10px;">
                    Powered by Cloudflare Workers with region bypass,<br>
                    mobile authentication headers, video streaming support,<br>
                    and category browsing for Anime, K-Drama, Kids content & more
                </p>
            </div>
        </div>
    </div>
    ...
    async fetch(request, env, ctx) {
        ...
        try {
            ...
            if (url.pathname.startsWith('/api/category/')) {
                const categoryPath = url.pathname.split('/api/category/')[1];
                return await handleCategory(decodeURIComponent(categoryPath));
            }
